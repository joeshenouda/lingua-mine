...
from lingua.metrics import MetricLogger
...
def launch_eval(cfg: EvalArgs):
    ...
    if get_global_rank() == 0:
        ...
        if val_results is not None:
            ...
            if cfg.tensorboard is not None and cfg.global_step is not None:
                from torch.utils.tensorboard import SummaryWriter
                tb_writer = SummaryWriter(log_dir=str(Path(cfg.dump_dir) / "tb_validation"))
                for source, metrics in val_results.items():
                    for metric_name, value in metrics.items():
                        if isinstance(value, (int, float)):
                            tb_writer.add_scalar(f"validation/{source}/{metric_name}", value, cfg.global_step)
                tb_writer.close()
