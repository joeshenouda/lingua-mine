...
def distribute_data_to_rank(dataset_path: str, rank: int, world_size: int, file_pattern: str):
    ...
    # === To check what shard each rank receives ===
    import hashlib
    import torch.distributed as dist

    assignment = rank_to_jsonl_iterator_params[rank]

    logger.info("***JOE: assignment: {}***".format(assignment))

    # Handle both dicts and JSONLState objects
    if isinstance(assignment, dict):
        file_path = assignment.get("file_path", "")
        offset = assignment.get("offset", "")
        block_size = assignment.get("block_size", "")
    else:
        file_path = getattr(assignment, "file_path", "")
        offset = getattr(assignment, "offset", "")
        block_size = getattr(assignment, "block_size", "")

    # Build the fingerprint string
    payload = f"{file_path}|{offset}|{block_size}".encode("utf-8")
    fingerprint = hashlib.sha256(payload).hexdigest()

    # Output directory for fingerprints
    outdir = os.environ.get("LINGUA_FINGERPRINT_DIR", "/tmp/lingua_fingerprints")
    ...
    # Write one file per rank
    with open(f"{outdir}/rank{global_rank}_shardhash.txt", "w") as f:
        f.write(f"file={file_path}\n")
        f.write(f"offset={offset}\n")
        f.write(f"block_size={block_size}\n")
        f.write(f"hash={fingerprint}\n")

    # Optional: print only from rank 0
    if dist.is_initialized() and dist.get_rank() == 0:
        print(f"[LINGUA DEBUG] Saved shard fingerprints to {outdir}")
    # === END LINGUA DEBUG ===
