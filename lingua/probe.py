...
def _get_stats(x_: torch.Tensor, remove_inf=False) -> Dict[str, Any]:
    ...
    # if len(tuple(x_.shape)) == 2:
    #     fan_out, fan_in = x_.shape

    #     # Power iteration for top singular value (spectral norm) in fp32
    #     with torch.no_grad():
    #         A = x_.float()  # fp32 for stability + bf16/fp16 support
    #         v = torch.randn(fan_in, device=A.device, dtype=A.dtype)
    #         v = v / (v.norm() + 1e-12)

    #         iters = 10 # tune: 6-12 typically good; 8 is a solid default
    ...
    #         # Rayleigh quotient estimate of sigma_max
    #         sigma = (u @ (A @ v))

    #     # RMS-induced operator norm scaling
    #     scale = x_.new_tensor(math.sqrt(fan_in / fan_out))
    #     spec_rms = (scale * sigma).to(dtype=x_.dtype)

    # elif len(tuple(x_.shape)) == 1:
    #     spec_rms = torch.linalg.norm(x_, ord=float("inf"))
    # else:
    #     spec_rms = x_.new_tensor(0.0)

    return {
        "shape": tuple(x_.shape),
        "mean": mean,
        "std": std,
        "rms_norm": torch.sqrt(torch.mean(x**2)),
