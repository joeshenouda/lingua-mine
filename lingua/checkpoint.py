...
class CheckpointManager:
    ...
    def load(
        ...
        # If continue_training_from_init is True, use init_ckpt_path instead of latest checkpoint
        if self.continue_training_from_init and self.init_ckpt_path:
            path = Path(self.init_ckpt_path)
        else:
            # Loading tries to load the provided path, if not available the last saved step
        ...
        train_state_name = TRAIN_STATE_NAME.format(dp_rank)
        train_state_file = path / train_state_name
